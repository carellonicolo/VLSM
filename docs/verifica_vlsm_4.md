---
**ITIS G. Marconi — Verona | Sistemi e Reti (SRI) | Prof. N. Carello | A.S. 2025/2026**

---
# VERIFICA SCRITTA — VLSM *(30 punti)*

| Alunno/a | Classe | Data |
|----------|--------|------|
| &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; | &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; | &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; |

---

## Esercizio 1 — VLSM: allocazione sottoreti *(10 punti — 2 pt per sottorete)*

Applicare il VLSM alla rete **`172.25.0.0/17`** per soddisfare i requisiti indicati.
Ordinare le sottoreti dalla più grande alla più piccola e completare la tabella di allocazione.

**Requisiti:**

| Sottorete | Host richiesti |
|-----------|---------------|
| LAN_A | 8000 |
| LAN_B | 4000 |
| LAN_C | 2000 |
| LAN_D | 1000 |
| LAN_E | 500  |

**Tabella di allocazione:**

| Sottorete | Ind. di rete | Prefisso | Maschera | Primo host | Ultimo host | Broadcast |
|-----------|-------------|----------|----------|------------|-------------|-----------|
| LAN_A | | | | | | |
| LAN_B | | | | | | |
| LAN_C | | | | | | |
| LAN_D | | | | | | |
| LAN_E | | | | | | |

---

## Esercizio 2 — VLSM: piano multi-segmento *(8 punti — 1 pt per sottorete)*

Applicare il VLSM alla rete **`10.50.0.0/21`** per soddisfare i requisiti indicati.
Ordinare le sottoreti dalla più grande alla più piccola e completare la tabella di allocazione.

**Requisiti:**

| Sottorete | Host richiesti |
|-----------|---------------|
| LAN_A | 250 |
| LAN_B | 100 |
| LAN_C | 60  |
| LAN_D | 25  |
| LAN_E | 10  |
| LAN_F | 5   |
| WAN_1 | 2   |
| WAN_2 | 2   |

**Tabella di allocazione:**

| Sottorete | Ind. di rete | Prefisso | Maschera | Primo host | Ultimo host | Broadcast |
|-----------|-------------|----------|----------|------------|-------------|-----------|
| LAN_A | | | | | | |
| LAN_B | | | | | | |
| LAN_C | | | | | | |
| LAN_D | | | | | | |
| LAN_E | | | | | | |
| LAN_F | | | | | | |
| WAN_1 | | | | | | |
| WAN_2 | | | | | | |

---

## Esercizio 3 — Parametri di sottorete *(7 punti — 1 pt per riga)*

Per ciascun indirizzo host/prefisso, determinare: indirizzo di rete, maschera decimale, primo host, ultimo host e broadcast.

| Ind. host / prefisso | Ind. di rete | Maschera decimale | Primo host | Ultimo host | Broadcast |
|----------------------|-------------|-------------------|------------|-------------|-----------|
| `10.77.200.15/12`     | | | | | |
| `172.30.100.200/22`   | | | | | |
| `192.168.17.90/28`    | | | | | |
| `10.120.64.130/18`    | | | | | |
| `172.16.135.200/20`   | | | | | |
| `10.200.100.100/21`   | | | | | |
| `192.168.255.253/30`  | | | | | |

---

## Esercizio 4 — Analisi piano VLSM di un'infrastruttura multi-sede *(5 punti)*

Un'organizzazione ha suddiviso il blocco **`10.0.0.0/16`** tra sei sedi, come indicato di seguito.

### a) *(3 pt)* — Completare la tabella con maschera, primo host, ultimo host e broadcast per ciascuna sede.

| Sede | Ind. di rete | Prefisso | Maschera | Primo host | Ultimo host | Broadcast |
|------|-------------|----------|----------|------------|-------------|-----------|
| A    | 10.0.0.0    | /18 | | | | |
| B    | 10.0.64.0   | /19 | | | | |
| C    | 10.0.96.0   | /20 | | | | |
| D    | 10.0.112.0  | /21 | | | | |
| E    | 10.0.120.0  | /22 | | | | |
| F    | 10.0.124.0  | /23 | | | | |

### b) *(1 pt)* — Identificare i blocchi non allocati nel blocco `10.0.0.0/16` ed esprimerli in notazione CIDR.

| Ind. di rete (CIDR) | Prefisso | Primo host | Ultimo host | Broadcast |
|---------------------|----------|------------|-------------|-----------|
| | | | | |
| | | | | |

### c) *(1 pt)* — Deve essere aggiunta una nuova sede che richiede **3000 host**. Indicare il blocco residuo da utilizzare, il prefisso CIDR minimo necessario e compilare la riga seguente.

| Ind. di rete | Prefisso | Maschera | Primo host | Ultimo host | Broadcast |
|-------------|----------|----------|------------|-------------|-----------|
| | | | | | |

---
*Nicolò Carello — info@nicolocarello.it*
