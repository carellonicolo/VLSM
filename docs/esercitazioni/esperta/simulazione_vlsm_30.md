...
Applicare il VLSM alla rete 172.224.0.0/11 per soddisfare i requisiti indicati.
...
Applicare il VLSM alla rete 10.100.0.0/16 per soddisfare i requisiti indicati.
Ordinare le sottoreti dalla più grande alla più piccola e completare la tabella di allocazione.
...
Un amministratore ha allocato le seguenti sottoreti all'interno del blocco 10.96.0.0/12.
...
### b) *(1 pt)* — Identificare i blocchi non allocati nel blocco `10.96.0.0/12` ed esprimerli in notazione CIDR.
...
### c) *(1 pt)* — È richiesta una nuova sottorete per **39.000 host**. Scegliere il blocco residuo più adatto e compilare la riga seguente.
